...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Observability Command', path: '/', icon: Eye },
    { name: 'Traffic Flow Analytics', path: '/flows', icon: ActivitySquare },
    { name: 'Latency & Performance', path: '/performance', icon: Signal },
    { name: 'Distributed Tracing', path: '/traces', icon: Crosshair },
    { name: 'Network Topology Map', path: '/topology', icon: Map },
    { name: 'Anomaly Detection', path: '/anomalies', icon: AlertTriangle },
    { name: 'SLO / SLA Tracking', path: '/slo', icon: FileText },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-indigo-400 bg-clip-text text-transparent">Net.Obs</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Senior NetEng</p>
                <p className="text-xs text-slate-500 truncate">SRE / Observability</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search trace IDs, source IP, or latency anomalies..." 
